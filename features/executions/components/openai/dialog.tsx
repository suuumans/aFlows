...
export const OpenAIDialog = ({ open, onOpenChange, onSubmit, defaultValues = {} }: Props) => {
    ...
    const watchVariableName = form.watch("variableName") || "openaiTest";

    const handleSubmit = (data: OpenAIFormValues) => {
        onSubmit(data);
        onOpenChange(false);
    }
                    ...
                    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6 mt-4">
                        <FormField control={form.control} name="variableName" render={({field}) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Input placeholder="openaiTest" {...field} />
                                </FormControl>
                                <FormDescription>
                        ...
                        <FormField control={form.control} name="model" render={({field}) => (
                            <FormItem>
                                ...
                                <FormDescription>
                                    Select the model to use for the OpenAI
                                </FormDescription>
                                <FormMessage />
                            </FormItem>
                        )} />
                        {/* OpenAI system prompt field */}
                        <FormField control={form.control} name="systemPrompt" render={({field}) => (
                            <FormItem>
                                ...
                                <FormDescription>
                                    System prompt sets the behavior of the OpenAI model. Use {"{{variables}}"} for simple values or {"{{json variables}}"} to stringify objects
                                </FormDescription>
                                <FormMessage />
                            </FormItem>
                        )} />
